...
jsonform.elementTypes = {
  ...
  'fieldset': {
    'template': '<fieldset class="form-group jsonform-error-<%= keydash %> <% if (elt.expandable) { %>expandable<% } %> <%= elt.htmlClass?elt.htmlClass:"" %>" ' +
      '<% if (id) { %> id="<%= id %>"<% } %>' +
      '>' +
      '<% if (node.title || node.legend) { %><legend id="<%= (node.title || node.legend) %>" legend="<%= node.legend %>"> <% if (!node.formElement.notitle) { %> <%= S(node.title || node.legend).humanize().s %> <% } %> <span style="font-size: 15px; font-style: italic;"><%= node.valuePreview %></span> </legend><% } %>' +
		...
		onBeforeRender: function (data, node) {
			...
			if (node.ownerTree.formDesc.value) {
				if (node.formElement.valuePath) {
					value = getDescendantProp(node.ownerTree.formDesc.value, node.formElement.valuePath)
				}
				if (!value) {
					value = {}
