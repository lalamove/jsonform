...
jsonform.fieldTemplate = function(inner) {
  return '<div ' +
    ...
    '">' +
    '<% if (!elt.notitle) { %>' +
      '<label style="width: 20%;" for="<%= node.id %>"><%= node.title ? node.title : node.name %>:</label>' +
    '<% } %>' +
		'<div class="controls" style="width: 80%; display: flex; align-items: center;">' +
			inner +
      // '<% if (node.description) { %>' +
...
var inputFieldTemplate = function (type) {
	return {
		'template': 
			...
			'<div style="width: 95%; display: flex; align-items: center;" class="<% if (node.prepend) { %>input-group<% } %>' +
				'<% if (node.append) { %> input-group<% } %>">' +
				'<% if (node.prepend) { %>' +
					'<span class="input-group-addon"><%= node.prepend %></span>' +
				'<% } %>' +
				'<span id="_jsonform-input-no-value-text" style="display: none;" ><%= escape("<NO VALUE>") %></span>' + 
				'<input type="' + type + '" ' +
				...
				' />' +
				'<% if (node.append) { %>' +
					'<span style="width: 30%" class="input-group-addon"><%= node.append %></span>' +
				'<% } %>' +
				'<% if (node.description) { %>' +
					'<i style="color: #448aff; margin: 0px 10px 0px 10px;" class="glyphicon glyphicon-question-sign" title="<%= node.description %>"></i>' +
				'<% } %>' +
				'<% if (!node.readOnly) { %>' +
					'<a href="#" name="<%= node.id %>" class="btn btn-default _jsonform-input-add" style="color: green; display: none"><i class="glyphicon glyphicon-plus-sign" title="Add value"></i></a>' +
					'<a href="#" name="<%= node.id %>" class="btn btn-default _jsonform-input-delete" style="color: red;"><i class="glyphicon glyphicon-remove-sign" title="Delete value"></i></a>' +
				'<% } %>' +
			'<% } %>' +
			'<% if (node.prepend || node.append) { %>' +
				'</div>' +
			...
			'<% if (!node.prepend && !node.append) { %>' +
				'<span id="_jsonform-input-no-value-text" style="display: none;" ><%= escape("<NO VALUE>") %></span>' + 
				'<input style="width: 90%" type="' + type + '" ' +
					'class=\'form-control<%= (fieldHtmlClass ? " " + fieldHtmlClass : "") %>\'' +
					'name="<%= node.name %>" value="<%= escape(value) %>" id="<%= id %>"' +
					'<%= (node.disabled? " disabled" : "")%>' +
					'<%= (node.readOnly ? " readonly=\'readonly\'" : "") %>' +
				...
				'<% } %>' +
				'<% if (!node.readOnly) { %>' +
					'<a href="#" id="<%= node.name %>" name="<%= node.id %>" class="btn btn-default _jsonform-input-add" style="color: green; display: none"><i class="glyphicon glyphicon-plus-sign" title="Add value"></i></a>' +
					'<a href="#" id="<%= node.name %>" name="<%= node.id %>" class="btn btn-default _jsonform-input-delete" style="color: red;"><i class="glyphicon glyphicon-remove-sign" title="Delete value"></i></a>' +
				'<% } %>' +
			'<% } %>'
			,
		'fieldtemplate': true,
		...
		onInsert: function (evt, node) {
			$(node.el).find('.' + escapeSelector('input-group-addon') + ' > input').css("width", "100%");
			// button add action
			$('a._jsonform-input-add', node.el).click(function (evt) {
				evt.preventDefault();
				evt.stopPropagation();
				$(node.el).find('.' + escapeSelector(node.id)).show()
				$(node.el).find('#' + escapeSelector(node.id)).attr('type', type)
				$(node.el).find('#' + escapeSelector('_jsonform-input-no-value-text')).hide()
				$(node.el).find('.' + escapeSelector('_jsonform-input-add')).hide()
				$(node.el).find('.' + escapeSelector('_jsonform-input-delete')).show()
				if (node.append) {
					$(node.el).find('.' + escapeSelector('input-group-addon')).show()
				}
				$(node.el).find('.' + escapeSelector('twitter-typeahead')).show()
				
				if (node.schemaElement.type === 'datetime') {
					$(node.el).find('#' + escapeSelector(node.id)).datetimepicker({
						sideBySide: true,
					})
				}
			});
			...
			$('a._jsonform-input-delete', node.el).click(function (evt) {
				evt.preventDefault();
				evt.stopPropagation();
				$(node.el).find('#' + escapeSelector(node.id)).attr('type','hidden')
				$(node.el).find('.' + escapeSelector(node.id)).hide()
				$(node.el).find('.' + escapeSelector('_jsonform-input-add')).show()
				$(node.el).find('.' + escapeSelector('_jsonform-input-delete')).hide()
				$(node.el).find('#' + escapeSelector('_jsonform-input-no-value-text')).show()
				if (node.append) {
					$(node.el).find('.' + escapeSelector('input-group-addon')).hide()
				}
				if (node.schemaElement.nullable || !node.schemaElement.required) {
					$(node.el).find('#' + escapeSelector(node.id)).val(null)
				} else {
					$(node.el).find('#' + escapeSelector(node.id)).val('')
				}
				$(node.el).find('.' + escapeSelector('twitter-typeahead')).hide()
			});

			// display condition
			// if no value and not required hide input field
			if (!node.value && !node.schemaElement.required) {
				$(node.el).find('#' + escapeSelector('_jsonform-input-no-value-text')).show()
				$(node.el).find('#' + escapeSelector(node.id)).val(null)
				$(node.el).find('#' + escapeSelector(node.id)).attr('type','hidden')
				$(node.el).find('.' + escapeSelector(node.id)).hide()
			...
			} else if (node.schemaElement.required) {
				$(node.el).find('.' + escapeSelector('_jsonform-input-add')).hide()
				$(node.el).find('.' + escapeSelector('_jsonform-input-delete')).hide()
			}
			$(node.el).find('#' + escapeSelector(node.id)).removeAttr('required');
			if (node.schemaElement.type === 'date') {
				(node.ownerTree.formDesc.rawSchema.properties[node.formElement.key] || {}).type = 'string'
			}
		}
	}
};
...
jsonform.elementTypes = {
    ...
    'onInsert': function(evt, node) {
      $(node.el).find('#' + escapeSelector(node.id)).spectrum({
        ...
        showInput: true
			...
			if (!node.value && !node.schemaElement.required) {
				$(node.el).find('#' + escapeSelector(node.id)).attr('type','hidden')
				$(node.el).find('.' + escapeSelector('sp-replacer')).hide()
			}
			...
			$('a._jsonform-input-add', node.el).click(function (evt) {
				...
				evt.stopPropagation();
				$(node.el).find('#' + escapeSelector('_jsonform-input-no-value-text')).hide()
				$(node.el).find('.' + escapeSelector('sp-replacer')).show()
				$(node.el).find('.' + escapeSelector('_jsonform-input-add')).hide()
				$(node.el).find('.' + escapeSelector('_jsonform-input-delete')).show()
			});
			...
			$('a._jsonform-input-delete', node.el).click(function (evt) {
				...
				$(node.el).find('.' + escapeSelector('_jsonform-input-add')).show()
				$(node.el).find('.' + escapeSelector('_jsonform-input-delete')).hide()
				if (node.schemaElement.nullable) {
					$(node.el).find('#' + escapeSelector(node.id)).val(null)
				} else {
					$(node.el).find('#' + escapeSelector(node.id)).val('')
				}
  ...
  'checkbox':{
		'template': '<div class="checkbox">' + 
				'<span id="_jsonform-input-no-value-text" style="display: none;" ><%= escape("<NO VALUE>") %></span>' + 
				'<input type="checkbox" style="position: relative; margin-left: 10px;" id="<%= id %>" ' +
				'<%= (fieldHtmlClass ? " class=\'" + fieldHtmlClass + "\'": "") %>' +
				...
				' />' +
				'<% if (node.inlinetitle) { %>' +
					'<label> node.inlinetitle </label>' +
				'<% } %>' +
				'<% if (node.schemaElement.default === undefined) { %>' +
					'<a href="#" name="<%= node.id %>" class="btn btn-default _jsonform-input-add" style="color: green; display: none"><i class="glyphicon glyphicon-plus-sign" title="Add value"></i></a>' +
					'<a href="#" name="<%= node.id %>" class="btn btn-default _jsonform-input-delete" style="color: red;"><i class="glyphicon glyphicon-remove-sign" title="Delete value"></i></a>' +
				'<% } %>' +
			'</div>',
    ...
    'getElement': function (el) {
      return $(el).parent().get(0);
		...
		onInsert: function (evt, node) {
			if (node.value === undefined && !node.schemaElement.required) {
				$(node.el).find('#' + escapeSelector(node.id)).attr('type','hidden')
				$(node.el).find('#' + escapeSelector(node.id)).val('<NO VALUE>')
				$(node.el).find('#' + escapeSelector('_jsonform-input-no-value-text')).show()
				$(node.el).find('.' + escapeSelector('_jsonform-input-delete')).hide()
				$(node.el).find('.' + escapeSelector('_jsonform-input-add')).show()
			}
			...
			$('a._jsonform-input-add', node.el).click(function (evt) {
				evt.preventDefault();
				evt.stopPropagation();
				$(node.el).find('#' + escapeSelector(node.id)).attr('type', 'checkbox')
				$(node.el).find('#' + escapeSelector(node.id)).val('0')
				$(node.el).find('#' + escapeSelector('_jsonform-input-no-value-text')).hide()
				$(node.el).find('.' + escapeSelector('_jsonform-input-add')).hide()
				$(node.el).find('.' + escapeSelector('_jsonform-input-delete')).show()
			});
  ...
  'select':{
		'template': '<select name="<%= node.name %>" id="<%= id %>"' +
			...
			'<%= (node.schemaElement && node.schemaElement.required ? " required=\'required\'" : "") %>' +
			'> ' +
			'<%= node.schemaElement.required ? "" : node.options.push(escape("<NO VALUE>"))%>' +
			'<% _.each(node.options, function(key, val) { if(key instanceof Object) { if (value === key.value) { %> <option selected value="<%= key.value %>"><%= key.title %></option> <% } else { %> <option value="<%= key.value %>"><%= key.title %></option> <% }} else { if (value === key) { %> <option selected value="<%= key %>"><%= key %></option> <% } else { %><option value="<%= key %>"><%= key %></option> <% }}}); %> ' +
			'</select>' + 
			'<span id="_jsonform-input-no-value-text" style="display: none;" ><%= escape("<NO VALUE>") %></span>' +
			'<% if (!node.schemaElement.disabled) { %>' +
				'<a href="#" name="<%= node.id %>" class="btn btn-default _jsonform-input-add" style="color: green; display: none"><i class="glyphicon glyphicon-plus-sign" title="Add value"></i></a>' +
				'<a href="#" name="<%= node.id %>" class="btn btn-default _jsonform-input-delete" style="color: red;"><i class="glyphicon glyphicon-remove-sign" title="Delete value"></i></a>' +
			'<% } %>'
			,
    'fieldtemplate': true,
		...
		onInsert: function (evt, node) {
			...
			$('a._jsonform-input-add', node.el).click(function (evt) {
				evt.preventDefault();
				evt.stopPropagation();
				$(node.el).find('#' + escapeSelector(node.id)).val(node.options[0])
				$(node.el).find('#' + escapeSelector(node.id) + ' > option:last').hide()
				$(node.el).find('#' + escapeSelector('_jsonform-input-no-value-text')).hide()
				$(node.el).find('.' + escapeSelector('_jsonform-input-add')).hide()
				$(node.el).find('.' + escapeSelector('_jsonform-input-delete')).show()
				$(node.el).find('#' + escapeSelector(node.id)).show()
  ...
  'array': {
    'template': '<div style="width: 100%" id="<%= id %>"><ul class="_jsonform-array-ul" style="list-style-type:none;"><%= children %></ul>' +
      '<span class="_jsonform-array-buttons">' +
        '<a href="#" id="<%= node.title %>" class="btn btn-default _jsonform-array-addmore"><i class="glyphicon glyphicon-plus-sign" title="Add new"></i></a> ' +
      ...
      if ($('').sortable) {
        ...
        return '<li data-idx="<%= node.childPos %>">' +
          '<span class="draggable line"><i class="glyphicon glyphicon-list" title="Move item"></i></span>' +
					inner +
					'<a href="#" name="<%= node.id %>" data-idx="<%= node.childPos %>" class="btn btn-default _jsonform-array-deleteitem"><i data-idx="<%= node.childPos %>" class="glyphicon glyphicon-minus-sign" title="Delete last"></i></a>' +
					'<% if (node.childPos > 0) { %>' +
						'<a href="#" name="<%= node.id %>" data-idx="<%= node.childPos %>" class="btn btn-default _jsonform-move-up"><i data-idx="<%= node.childPos %>" class="glyphicon glyphicon-arrow-up" title="Move up"></i></a>' +
					'<% } %>' +
					'<a href="#" name="<%= node.id %>" data-idx="<%= node.childPos %>" class="btn btn-default _jsonform-move-down"><i data-idx="<%= node.childPos %>" class="glyphicon glyphicon-arrow-down" title="Move down"></i></a>' +
          '</li>';
      }
      else {
        return '<li data-idx="<%= node.childPos %>">' +
    ...
    'onInsert': function (evt, node) {
      var $nodeid = $(node.el).find('#' + escapeSelector(node.id));
			...
			if (node.children.length === 1) {
				if (boundaries.minItems <= 0 && !node.ownerTree.formDesc.value) {
					node.deleteArrayItem(0);
				}
			}
      ...
      var moveNodeTo = function (fromIdx, toIdx) {
        ...
        if (fromIdx < toIdx) {
          ...
          $(node.children[fromIdx-1].el).after(fromEl);
			...
			$('#' + escapeSelector(node.id) + ' > ul > li a._jsonform-move-down').last().hide()
        ...
        if (boundaries.maxItems >= 0) {
          ...
          if (node.children.length > boundaries.maxItems - 1) {
            return false;
				...
				$('#' + escapeSelector(node.id) + ' > ul > li a._jsonform-move-down').last().show()
      ...
      if ((boundaries.minItems > 0) &&
        ...
        $nodeid.find('a._jsonform-array-deleteitem')
          .addClass('disabled');
			...
			$($nodeid).on( "click", "a._jsonform-array-deleteitem", function(evt) {
				evt.preventDefault();
				evt.stopPropagation();
				var idx = parseInt(evt.target.getAttribute('data-idx'))
				if (boundaries.minItems > 0) {
        ...
        if ((boundaries.maxItems >= 0) && (idx <= boundaries.maxItems - 1)) {
          $nodeid.find('> span > a._jsonform-array-addmore')
			...
			$($nodeid).on( "click", "a._jsonform-move-up", function(evt) {
				evt.preventDefault();
				evt.stopPropagation();
				var idx = parseInt(evt.target.getAttribute('data-idx'))
				var newIdx = idx - 1;
			...
			$($nodeid).on( "click", "a._jsonform-move-down", function(evt) {
				evt.preventDefault();
				evt.stopPropagation();
				var idx = parseInt(evt.target.getAttribute('data-idx'))
				var newIdx = idx + 1;
  ...
  'fieldset': {
    'template': '<fieldset class="form-group jsonform-error-<%= keydash %> <% if (elt.expandable) { %>expandable<% } %> <%= elt.htmlClass?elt.htmlClass:"" %>" ' +
      '<% if (id) { %> id="<%= id %>"<% } %>' +
      '>' +
      '<% if (node.title || node.legend) { %><legend id="<%= (node.title || node.legend) %>" legend="<%= node.arrayPath[0] %>" parent="<%= node.parentNode.title + "-" + node.parentNode.arrayPath[0] %>"> <% if (!node.formElement.notitle) { %> <%= S(node.title || node.legend).humanize().s %> <% } %> <span style="font-size: 15px; font-style: italic;"><%= node.valuePreview %></span> </legend><% } %>' +
      '<% if (elt.expandable) { %><div style="margin-left: 1em;" class="form-group"><% } %>' +
      '<%= children %>' +
      '<% if (elt.expandable) { %></div><% } %>' +
			'</fieldset>',
		onBeforeRender: function (data, node) {
			const getDescendantProp = (node) => {
				let obj = node.ownerTree.formDesc.value
				const desc = node.formElement.valuePath
				var arr = desc.split(".");
				while(arr.length) {
					let idx
					let key = arr.shift()
					if (key.includes('[]')) {
						key = key.replace(/[\[\]']+/g,'')
						idx = node.parentNode.childPos
					}
					const value = idx === undefined ? obj = obj[key] : (obj[key] || [])[idx]
					if (!value) {
						break
					}
					obj = value				
			...
			let value
			if (node.ownerTree.formDesc.value) {
				if (node.formElement.valuePath) {
					value = getDescendantProp(node)
				}
				if (!value) {
					value = {}
					node.children.forEach(i => {
						if (i.key) {
							value[i.key.split('.').pop()] = i.value
						}
					})
				}
			}
			...
			if (!_.isArray(value)) {
				...
				node.valuePreview = value.length > 100 ? value.substring(0, 100) + "...}" : value;
				return
			}
			node.valuePreview = `${value.length} ${value.length > 1 ? 'items' : 'item'}`
		},
    onInsert: function (evt, node) {
      if (node.el !== null) {
...
formNode.prototype.getFormValues = function (updateArrayPath) {
  ...
  for (var i = 0; i < formArray.length; i++) {
    ...
    if (eltSchema.type === 'boolean') {
      ...
      } else if (formArray[i].value === '<NO VALUE>') {
    ...
    if ((eltSchema.type === 'string') &&
      (formArray[i].value === '' || formArray[i].value === '<NO VALUE>') &&
      !eltSchema._jsonform_allowEmpty) {
      formArray[i].value=null;
		}
		if ((eltSchema.type === 'datetime') &&
      (formArray[i].value === '' || formArray[i].value === '<NO VALUE>') &&
      !eltSchema._jsonform_allowEmpty) {
      formArray[i].value=null;
		} 
		if ((eltSchema.type === 'datetime' && formArray[i].value !== null) && (formArray[i].value !== '' && formArray[i].value !== '<NO VALUE>')) {
			const utcDate = new Date(formArray[i].value)
			formArray[i].value = utcDate.toISOString()
		}
    ...
    if ((eltSchema.type === 'object') &&
      ...
      formArray[i].value = null;
		...
		// add color type
		if ((eltSchema.type === 'color') &&
      (formArray[i].value === '') &&
      !eltSchema._jsonform_allowEmpty) {
      formArray[i].value=null;
    }

		// set value if field is nullable or has value
    if ((formArray[i].name && formArray[i].value !== null) || eltSchema.nullable) {
...
var removeNoValue = function (values) {
	Object.keys(values).forEach(k => {
		if (_.isArray(values[k])) {
			removeNoValue(values[k])
			return
		}
		if (_.isObject(values[k])) {
			removeNoValue(values[k])
  ...
  if (options.validate!==false) {
    ...
    if (typeof options.validate!="object") {
      // if (global.JSONFormValidator) {
			...
			// }
			validator = new Ajv({ useDefaults: true, format: "full", coerceTypes: true, nullable: true })
    } else {
      validator = options.validate;
    }
    if (validator) {
			const validate = validator.compile(this.formDesc.rawSchema || this.formDesc.schema);
			const valid = validate(values)
			$(this.domRoot).jsonFormErrors(false,options);
			if (!valid) {
				if (validate.errors.length) {
					errors = validate.errors.map(i => { 
						i.uri = (i.params[Object.keys(i.params)[0]] || '').toString()
						i.detail = i.message
						return i
					});
				}
 ...
 */
$.fn.llmpJsonForm = function(options) {
  var formElt = this;
  ...
  formElt.on('click', '.expandable > legend', function () {
    var parent = $(this).parent();
		...
		var values = jsonform.getFormValue(form.domRoot)
		var id = this.id
		let value = values[id]
		const idx = $(this).attr('legend')
		if (!value) {
		...
		if (_.isArray(value)) {
			...
			$(this).find('span').text(`${items} ${items > 1 ? 'items' : 'item'}`)
