...
jsonform.elementTypes = {
  ...
  'fieldset': {
    'template': '<fieldset class="form-group jsonform-error-<%= keydash %> <% if (elt.expandable) { %>expandable<% } %> <%= elt.htmlClass?elt.htmlClass:"" %>" ' +
      '<% if (id) { %> id="<%= id %>"<% } %>' +
      '>' +
      '<% if (node.title || node.legend) { %><legend id="<%= (node.title || node.legend).toLowerCase() %>"><%= node.title || node.legend %> <span style="font-size: 15px; font-style: italic;"><%= node.valuePreview %></span> </legend><% } %>' +
  ...
  formElt.on('click', '.expandable > legend', function () {
    var parent = $(this).parent();
		...
		if (_.isArray(values[id])) {
			const items = values[id].length
			$(this).find('span').text(`${items} ${items > 1 ? 'items' : 'item'}`)
		} else if (_.isObject(values[id])) {
			let value = JSON.stringify(values[id])
