...
jsonform.elementTypes = {
  ...
  'fieldset': {
    'template': '<fieldset class="form-group jsonform-error-<%= keydash %> <% if (elt.expandable) { %>expandable<% } %> <%= elt.htmlClass?elt.htmlClass:"" %>" ' +
      '<% if (id) { %> id="<%= id %>"<% } %>' +
      '>' +
      '<% if (node.title || node.legend) { %><legend id="<%= (node.title || node.legend) %>" legend="<%= node.arrayPath[0] %>" parent="<%= node.parentNode.title + "-" + node.parentNode.arrayPath[0] %>"> <% if (!node.formElement.notitle) { %> <%= humanize(node.title || node.legend) %> <% } %> <span style="font-size: 15px; font-style: italic;"><%= node.valuePreview %></span> </legend><% } %>' +
