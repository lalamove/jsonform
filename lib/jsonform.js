...
var inputFieldTemplate = function (type) {
	return {
		'template': 
			...
			'<div style="width: 100%; display: flex; align-items: center;" class="<% if (node.prepend) { %>input-group<% } %>' +
				...
				'<span class="<%= id %>" style="position: relative; display: inline-block; width: 80%; padding: 0px 10px 0px 10px;">' +
				'<input type="' + type + '" ' +
...
jsonform.elementTypes = {
  ...
  'select':{
		'template': '<span class="<%= id %>" style="position: relative; display: inline-block; width: 80%; padding: 0px 10px 0px 10px;">' +
  ...
  'fieldset': {
    'template': '<fieldset class="form-group jsonform-error-<%= keydash %> <% if (elt.expandable) { %>expandable<% } %> <%= elt.htmlClass?elt.htmlClass:"" %>" ' +
      '<% if (id) { %> id="<%= id %>"<% } %>' +
      '>' +
      '<% if (node.title || node.legend) { %><legend id="<%= (node.title || node.legend).toLowerCase() %>" legend="<%= node.legend %>"> <% if (!node.formElement.notitle) { %> <%= node.title || node.legend %> <% } %> <span style="font-size: 15px; font-style: italic;"><%= node.valuePreview %></span> </legend><% } %>' +
